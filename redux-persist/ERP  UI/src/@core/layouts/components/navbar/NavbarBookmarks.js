...
import { getBookmarks, handleSearchQuery, updateBookmarked } from '@store/actions/navbar';
// import { getBookmarks } from '../../../../redux/actions/navbar/Index';
...
import {
    DropdownItem, DropdownMenu, DropdownToggle, NavItem,
    NavLink, UncontrolledDropdown, UncontrolledTooltip
} from 'reactstrap';
...
const NavbarBookmarks = props => {
    ...
    // ** Loops through Bookmarks Array to return Boo kmarks
    const renderBookmarks = () => {
        if ( store.bookmarks.length ) {
            return store.bookmarks.filter( item => item.isBookmarked === true )
                .map( item => {
                    const IconTag = Icon[item.icon];
                    return (
                        <NavItem key={item.target} className='d-block'>
