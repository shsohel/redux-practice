...
const CustomFloat = ( { children, title } ) => {
    ...
    return (
        ...
        >
            {
                openCustomizer ? <Button.Ripple className='custom-float-toggle d-flex align-items-center justify-content-center' onClick={handleToggle} color='primary'>
                    <span className='align-middle ml-25'>{title}</span>
                    <ChevronsDown className='align-middle ml-25' size={18} />
                </Button.Ripple> : <Button.Ripple className='custom-float-toggle d-flex align-items-center justify-content-center rotateX' onClick={handleToggle} color='primary'>
                    <span className='align-middle ml-25'>{title}</span>
