...
import { ADD_ROLE, DELETE_ROLE, DELETE_ROLES_BY_RANGE, DROP_DOWN_ROLES, GET_ROLES, GET_ROLES_BY_QUERY, GET_ROLE_BY_ID, OPEN_ROLE_SIDEBAR, SELECTED_ROLE_NULL, UPDATE_ROLE } from "../actionTypes";
...
export const getAllRoles = () => {
    return async dispatch => {
        await baseAxios.get( `${merchandisingApi.role.get_roles}` ).then( response => {
            dispatch( {
                type: GET_ROLES,
                roles: response.data
            } );
...
export const getRoleByQuery = params => {
    ...
    return async dispatch => {
        await baseAxios.get( `${merchandisingApi.role.get_roles_by_query}`, params ).then( response => {
            dispatch( {
...
export const getRoleById = id => {
    return async dispatch => {
        await baseAxios
            .get( `${merchandisingApi.role.get_role_by_id}`, { id } )
            .then( response => {
                dispatch( {
                    type: GET_ROLE_BY_ID,
...
export const addRole = role => {
    return async ( dispatch, getState ) => {
        await baseAxios
            .post( `${merchandisingApi.role.add_role}`, role )
            .then( response => {
                dispatch( {
                    type: ADD_ROLE,
                    role
...
export const updateRole = role => {
    return ( dispatch, getState ) => {
        baseAxios
            .post( `${merchandisingApi.role.update_role}`, { role } )
            .then( response => {
                dispatch( {
                    type: UPDATE_ROLE,
                    role
...
export const deleteRole = id => {
    return ( dispatch, getState ) => {
        confirmDialog( confirmObj ).then( async e => {
            if ( e.isConfirmed ) {
                baseAxios
                    .delete( `${merchandisingApi.role.delete_role}`, { id } )
                    .then( response => {
                        dispatch( {
                            type: DELETE_ROLE
                        } );
...
export const deleteRangeRole = ids => {
    return ( dispatch, getState ) => {
        confirmDialog( confirmObj ).then( e => {
            if ( e.isConfirmed ) {
                baseAxios
                    .delete( `${merchandisingApi.role.delete_roles_by_range}`, { ids } )
                    .then( response => {
                        dispatch( {
