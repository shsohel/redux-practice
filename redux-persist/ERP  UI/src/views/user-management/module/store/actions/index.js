...
import { ADD_MODULE, DELETE_MODULE, DELETE_MODULES_BY_RANGE, DROP_DOWN_MODULES, GET_MODULES, GET_MODULES_BY_QUERY, GET_MODULE_BY_ID, OPEN_MODULE_SIDEBAR, SELECTED_MODULE_NULL, UPDATE_MODULE } from "../../../module/store/actionTypes";
...
export const getAllModules = () => {
    return async dispatch => {
        await baseAxios.get( `${merchandisingApi.module.get_modules}` ).then( response => {
            dispatch( {
                type: GET_MODULES,
...
export const getModuleByQuery = params => {
    ...
    return async dispatch => {
        await baseAxios.get( `${merchandisingApi.module.get_modules_by_query}`, params ).then( response => {
            dispatch( {
...
export const getModuleById = id => {
    return async dispatch => {
        await baseAxios
            .get( `${merchandisingApi.module.get_module_by_id}`, { id } )
            .then( response => {
                dispatch( {
...
export const addModule = module => {
    return async ( dispatch, getState ) => {
        await baseAxios
            .post( `${merchandisingApi.module.add_module}`, module )
            .then( response => {
                dispatch( {
                    type: ADD_MODULE,
                    module
...
export const updateModule = module => {
    return ( dispatch, getState ) => {
        baseAxios
            .post( `${merchandisingApi.module.update_module}`, { module } )
            .then( response => {
                dispatch( {
                    type: UPDATE_MODULE,
                    module
...
export const deleteModule = id => {
    return ( dispatch, getState ) => {
        confirmDialog( confirmObj ).then( async e => {
            if ( e.isConfirmed ) {
                baseAxios
                    .delete( `${merchandisingApi.module.delete_module}`, { id } )
                    .then( response => {
                        dispatch( {
                            type: DELETE_MODULE
                        } );
...
export const deleteRangeModule = ids => {
    return ( dispatch, getState ) => {
        confirmDialog( confirmObj ).then( e => {
            if ( e.isConfirmed ) {
                baseAxios
                    .delete( `${merchandisingApi.module.delete_modules_by_range}`, { ids } )
                    .then( response => {
                        dispatch( {
