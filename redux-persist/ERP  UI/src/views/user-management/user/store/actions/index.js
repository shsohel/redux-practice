...
import { ADD_USER, DELETE_USER, DELETE_USER_BY_RANGE, DROP_DOWN_USERS, GET_USERS, GET_USERS_BY_QUERY, GET_USER_BY_ID, OPEN_USER_SIDEBAR, SELECTED_USER_NULL, UPDATE_USER } from "../actionTypes";
...
export const getAllUsers = () => {
    return async ( dispatch ) => {
        await baseAxios
            .get( `${merchandisingApi.user.get_users}` )
            .then( ( response ) => {
                dispatch( {
                    type: GET_USERS,
                    users: response.data
...
export const getDropDownUsers = () => {
    return async ( dispatch ) => {
        await baseAxios
            .getAllUsers( `${merchandisingApi.user.get_users}` )
            .then( ( response ) => {
                dispatch( {
...
export const getUserByQuery = ( params ) => {
    return async ( dispatch ) => {
        await baseAxios
            .get( `${merchandisingApi.user.get_users_by_query}`, params )
            .then( ( response ) => {
                dispatch( {
...
export const getUserById = ( id ) => {
    return async ( dispatch ) => {
        await baseAxios
            .get( `${merchandisingApi.user.get_user_by_id}`, { id } )
            .then( ( response ) => {
                dispatch( {
                    type: GET_USER_BY_ID,
...
export const addUser = ( user ) => {
    return async ( dispatch, getState ) => {
        await baseAxios
            .post( `${merchandisingApi.user.add_user}`, user )
            .then( ( response ) => {
                dispatch( {
                    type: ADD_USER,
                    user
...
export const updateUser = ( user ) => {
    return ( dispatch, getState ) => {
        baseAxios
            .post( `${merchandisingApi.user.update_user}`, { user } )
            .then( ( response ) => {
                dispatch( {
                    type: UPDATE_USER,
                    user
...
export const deleteUser = ( id ) => {
    return ( dispatch, getState ) => {
        ...
        confirmDialog( confirmObj ).then( async e => {
            if ( e.isConfirmed ) {
                await baseAxios
                    .delete( `${merchandisingApi.user.delete_user}`, { id } )
                    .then( response => {
                        dispatch( {
                            type: DELETE_USER
                        } );
...
export const deleteRangeUser = ( ids ) => {
    return ( dispatch, getState ) => {
        ...
        confirmDialog( confirmObj ).then( e => {
            if ( e.isConfirmed ) {
                baseAxios
                    .delete( `${merchandisingApi.user.delete_users_by_range}`, { ids } )
                    .then( response => {
                        dispatch( {
