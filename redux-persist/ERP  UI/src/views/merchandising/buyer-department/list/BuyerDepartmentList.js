...
import { deleteRangeBuyerDepartment, getBuyerDepartmentByQuery, handleOpenBuyerDepartmentSidebar } from '../store/actions';
import BuyerDepartmentExpandRow from './BuyerDepartmentExpandRow';
import { BuyerDepartmentTableColumns } from './BuyerDepartmentTableColumn';


const BuyerDepartmentList = () => {
    const dispatch = useDispatch();
    // ** Global States
    const { buyerDepartments, total, selectedBuyerDepartment, openBuyerDepartmentSidebar, queryData } = useSelector( ( { buyerDepartments } ) => buyerDepartments );
    // ** Season States
    ...
    const dataToRender = () => {
        ...
        if ( queryData.length > 0 ) {
            ...
            return buyerDepartments.slice( 0, rowsPerPage );
        }
    };
    ...
    return (
        <div>
            ...
            <Card>
                <div >
                    <CardHeader>
                        <CardTitle tag='h2'>Buyer Department</CardTitle>
                    </CardHeader>
                    <TableCustomerHeader
                    ...
                    <DataTable
                        ...
                        onSort={handleSort}
                        // contextMessage={}
                        contextActions={<Button.Ripple onClick={() => { handleDeleteBuyerDepartmentRange(); }} className='btn-icon ' color='flat-danger'>
                            <XSquare size={24} />
                        </Button.Ripple>}
                        dense
                        subHeader={false}
                        ...
                        expandOnRowClicked
                        columns={BuyerDepartmentTableColumns}
            ...
            {
                selectedBuyerDepartment !== null && openBuyerDepartmentSidebar ? < BuyerDepartmentEditForm data={selectedBuyerDepartment} open={openBuyerDepartmentSidebar} toggleSidebar={toggleSidebar} /> : openBuyerDepartmentSidebar ? <BuyerDepartmentAddForm open={openBuyerDepartmentSidebar} toggleSidebar={toggleSidebar} /> : null
            }
        </div>
    );
};
