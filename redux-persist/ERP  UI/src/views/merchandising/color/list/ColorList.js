...
import { deleteRangeColor, getColorByQuery, handleOpenColorSidebar } from '../store/actions';
import ColorExpandRow from './ColorExpandRow';
import { ColorTableColumns } from './ColorTableColumn';
...
// ** Color Status filter options: Demo Array
const statusOptions = [
...
const ColorList = () => {
    ...
    // ** Global States
    const { colors, total, selectedColor, openColorSidebar, queryData } = useSelector( ( { colors } ) => colors );
    // ** Color States
    ...
    const dataToRender = () => {
        ...
        if ( queryData.length > 0 ) {
            ...
            return colors.slice( 0, rowsPerPage );
        }
    };
    ...
    return (
        <div>
            ...
            <Card>
                <div >
                    <CardHeader>
                        <CardTitle tag='h2'>Style Color</CardTitle>
                    </CardHeader>
                    <CustomHeader
                    ...
                    <DataTable
                        ...
                        paginationServer
                        expandableRows
                        expandableRowsComponent={<ColorExpandRow data={data => data} />}
                        expandOnRowClicked
            ...
            {
                selectedColor !== null ? < ColorEditForm data={selectedColor} open={openColorSidebar} toggleSidebar={toggleSidebar} /> : <ColorAddForm open={openColorSidebar} toggleSidebar={toggleSidebar} />
            }
        </div>
    );
};
